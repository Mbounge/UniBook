...
const Dropdown = ({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {isOpen && (
        <div
          className={`absolute z-20 top-full mt-2 bg-white rounded-lg border border-gray-200 overflow-hidden max-h-60 overflow-y-auto ${widthClass}`}
        >
          {options.map((option) => (
            <button
              key={option.value}
...
export const EditorToolbar: React.FC<EditorToolbarProps> = (props) => {
  const {
    ...
    onColorMenuOpen,
    onLineSpacingChange,
    onLineSpacingMenuOpen,
    onInsertMath, // <-- NEW PROP
    isTocOpen,
  } = props;
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm border border-gray-200/50 rounded-lg p-3 flex items-center flex-wrap gap-1">
      ...
      />
      <div className="h-6 w-px bg-gray-200 mx-2"></div>
      <ToolbarButton onClick={onBulletedList} title="Bulleted List">
        <List className="w-4 h-4" />
      </ToolbarButton>
      <ToolbarButton onClick={onNumberedList} title="Numbered List">
        <ListOrdered className="w-4 h-4" />
      </ToolbarButton>
      <div className="h-6 w-px bg-gray-200 mx-2"></div>
      <ToolbarButton onClick={onInsertImage} title="Add Image">
        <Image className="w-4 h-4" />
      </ToolbarButton>
      {/* --- NEW BUTTON ADDED HERE --- */}
      <ToolbarButton onClick={onInsertMath} title="Insert Formula">
